/*  
**    Suma Logaritmica: implementación en MPI de la versión de suma de un arreglo
**    usando el modelo maestro-trabajador para dividir el arreglo y realizar las sumas parciales
**    aplicando al final el algoritmo de suma logaritmica para la suma total
...
int main(int argc, char *argv[])
{
    ...
    long long sumaParcialRes = 0;   // Para recibir las sumas parciales

    int numeral;    // Número que le toca a cada proceso
    int numRondas, i, pot, recibo, envio;
    ...
    while (numeral % 2 == 0 && i < numRondas)
    {
        ...
        MPI_Recv(&sumaParcialRes, 1, MPI_LONG_LONG, recibo, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        sumaParcial += sumaParcialRes;    // Se van realizando las sumas parciales
        numeral = numeral/2;
        i++;
        pot = 2*pot;
    }
    ...
    {
        sumaTotal += sumaParcial;   // Realiza la suma total
